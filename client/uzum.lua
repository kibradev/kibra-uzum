	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				if zone == true then 
				TriggerEvent('cd_drawtextui:ShowUI', 'show', "E - Üzüm Topla")
				end
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				exports['aex-bar']:taskBar(5000,'Üzüm Toplanıyor')
...
YikaUzum = function()
	...
	if finished2 ~= 100 then
		TriggerServerEvent('kibra-uzum:SilUzum')
		ClearPedTasksImmediately(player)
		exports['mythic_notify']:SendAlert('error', 'Kirli Üzüm Ayıklayamadınız! 1 Tane Kirli üzümünüzü kaybettiniz.', 2000)
	else 
		TriggerServerEvent("kibra-uzum:UzumYikama")
	end
	end
Citizen.CreateThread(function()
while true do 
...
ESX.ShowHelpNotification('~INPUT_CONTEXT~ - Üzüm Yıka')
...
Citizen.CreateThread(function()
    if Config.BlipAcik then
      local blip = AddBlipForCoord(Config.SaticiBlip)
	  SetBlipSprite(blip, 478)
	  SetBlipDisplay(blip, 4)
	  SetBlipScale(blip, 0.6)
      ...
      SetBlipAsShortRange(blip, true)
	  BeginTextCommandSetBlipName("STRING")
      AddTextComponentString("Üzüm Satış")
      EndTextCommandSetBlipName(blip)
    end
end)
...
Citizen.CreateThread(function()
    if Config.BlipAcik then
      ...
      SetBlipAsShortRange(blip, true)
	  BeginTextCommandSetBlipName("STRING")
      AddTextComponentString("Üzüm Yıkama")
      EndTextCommandSetBlipName(blip)
    end
end)
