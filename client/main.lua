	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1.50 then
			if not menuOpen then
				ESX.ShowHelpNotification("Satıcıyla Konuş")
...
while true do 
	...
	for k,v in pairs(Config.Satici) do
	local distsat = #(kordinat - v)
	if distsat < 1 then 
		ESX.ShowHelpNotification('~INPUT_CONTEXT~ - Üzüm Satış')
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.Satici) do
    npc = CreatePed(1, GetHashKey("a_m_y_bevhills_01"), v.x, v.y, 28.3484, 306.74, false, true)
